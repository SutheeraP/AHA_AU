...
const page = () => {
  const staff = [
    { name: "Anonymous Hero", job: "CONTENT CREATOR", img: "staff1.jpg" },
    { name: "Sutheerap", job: "WEBSITE DEVELOPER", img: "staff2.jpg" },
    { name: "Lotte", job: "ILLUSTRATOR", img: "staff3.jpg" },
    { name: "Facee Hee", job: "ILLUSTRATOR", img: "staff4.jpg" },
  ];
  return (
          ...
          <div className="min-h-screen flex container mx-auto">
            <div className="w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 px-4 max-w-3xl mx-auto">
                {staff.map(({ name, job, img }) => (
                  <div key={name}>
                    <div className={`w-full aspect-[4/5] mb-2 rounded-lg`}></div>
                    <div>{name}</div>
                    <div className="text-faded font-light text-xs">{job}</div>
                  </div>
                ))}
              </div>
